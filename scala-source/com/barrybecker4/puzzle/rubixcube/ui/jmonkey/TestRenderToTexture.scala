...
object TestRenderToTexture {
  ...
  def main(args: Array[String]): Unit = {
    val app = new TestRenderToTexture
    app.start()
  }
}
...
class TestRenderToTexture extends SimpleApplication with ActionListener {
  ...
  override def simpleInitApp(): Unit = {
    ...
    inputManager.addMapping(TestRenderToTexture.TOGGLE_UPDATE, new KeyTrigger(KeyInput.KEY_SPACE))
    inputManager.addListener(this, TestRenderToTexture.TOGGLE_UPDATE)
  }
  ...
  override def onAction(name: String, isPressed: Boolean, tpf: Float): Unit = {
    if (name == TestRenderToTexture.TOGGLE_UPDATE && isPressed) offView.setEnabled(!offView.isEnabled)
  }
}