...
final class TopControls (controller: PuzzleController[Pails, PourOperation],
  ...
  override protected def addAdditionalControls(panel: JPanel): Unit = {
    ...
    firstPailSize =
      new NumberInput("First Pail Size", 9,
        "The fill capacity of the first container", 1, MAX_CAPACITY, true)
    secondPailSize =
      new NumberInput("Second Pail Size", 4,
        "The fill capacity of the second container", 1, MAX_CAPACITY, true)
    targetMeasure =
      new NumberInput("Target measure", 6,
        "The amount of liquid that is to be measured out exactly", 1,
        MAX_CAPACITY, true)
