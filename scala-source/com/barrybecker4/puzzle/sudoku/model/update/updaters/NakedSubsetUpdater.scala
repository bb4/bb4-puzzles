...
import com.barrybecker4.puzzle.sudoku.model.board.{Board, CellSet}
...
class NakedSubsetUpdater(val b: Board) extends AbstractUpdater(b) {
  ...
  /**
    ...
    *   n = num candidate digits currently determined for the cell
    *   if we can find n-1 other cells with only a subset of those n digits and no others {
  ...
  private def checkNakedSubsetInBigCells() {
    for (i <- 0 until board.getBaseSize; j <- 0 until board.getBaseSize)
  ...
  private def checkNakedSubset(cells: CellSet) {
    for (i <- 0 until cells.numCells) {
      ...
      matches += i
      if (!cands.isEmpty) {
        val numCandidates = cands.size
        for (j <- 0 until cells.numCells) {
          val cands2 = cells.getCell(j).getCandidates
          if (j != i && cands.containsAll(cands2))
            matches += j
        }
        if (matches.size == numCandidates) {
          for (i <- 0 until cells.numCells) {
            val cell = cells.getCell(i)
            if (!matches.contains(i)) cell.getCandidates.removeAll(cands)
          }
          return // done
        }
      }
